...
*owl-cli* uses Java 13 source features, but the build generates a Java 11-compatible JAR file. This
 means, for building the executable JAR file, you need Java *13*. The `gradle` build forks jobs
 where it expects Java 13 to be installed in a certain place (see <<building-native-image>> for an
...
*owl-cli* uses Java 13 source features, but the build generates a Java 11-compatible JAR file, which
 is required for the generation of the native image using https://www.graalvm.org/[GraalVM], as
 GraalVM currently does not support Java 13 class files. This means, for building the native image,
 you need both Java *11* (i.e. GraalVM JDK) *and* Java *13*. In particular, the `gradle` build must
 be executed using Java 11, while Java 13 is installed in a certain place where the build can
